...
- **Are you calling it data governance to an audience that will never buy that word?** `[getting-started]` `[frameworks]` — The words that reliably cost buy-in are "governance" and "committee" — "data management sync" works identically and lands better. "Certification" is not one of them: it is a normal word for a status with a lifecycle, and renaming it buys nothing. If the participant cannot restate the whole program without the discipline's name, they have not found the pain it attaches to.
