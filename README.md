...
| `skills/dg-econ-effect/SKILL.md` | The economic effect skill: builds the money model behind a strategy — three real ROI zones, three methods, the discount ladder, and a precision list that says what must be measured to replace the estimate with a calculation. Runs standalone, or as subsection `03.x` inside a strategy |
