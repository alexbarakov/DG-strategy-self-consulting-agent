...
2. **Confirm the content structure** — the default v2 skeleton (00 Context → 01 AS-IS on the Maturity Scorecard → 02 TO-BE beyond AI → 03 Streams → 04 Initiatives Portfolio with execution playbooks → 05 Metrics → 06 Operating Model → 07 Risks & Kill-gates) is shown for trimming/reordering before work starts.
3. **Volume** — default is a **6-pager**; options: full block-by-block wiki, or per-block delivery with review after each.
